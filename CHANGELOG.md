...
- Build the arm64 base image with `-DENABLE_VLC=OFF`, so dropping the `libvlc-dev` build dependency no longer fails OBS's CMake configure step. ([#148](https://github.com/adanalife/obs/pull/148))
...
- release-please runs on the automation app token, so the release tag fires `release.yml` by itself (no explicit dispatch) and the Discord announcement only fires on a tag push, never on a re-deploy. ([#143](https://github.com/adanalife/obs/pull/143))
...
- The four remaining `bin/obs-*` break-glass helpers now have tests. Making them testable meant giving each a `main()` — matching `obs-browser-refresh`, which already had that shape — so all five helpers are now one shape and their decisions are exercised in CI rather than first discovered mid-incident. ([#149](https://github.com/adanalife/obs/pull/149))
